...
export function WorkshopCard({ event, index = 0 }: WorkshopCardProps) {
  ...
  return (
    <motion.article
      ...
      className="group relative h-full overflow-hidden border border-gold/40 bg-[linear-gradient(160deg,#fdf8ec_0%,#f3e6c8_100%)] p-7 shadow-card transition-colors duration-300"
    >
      ...
      <div className="relative z-10 flex h-full flex-col justify-between">
        ...
        <Link
          to={`/events/${event.id}`}
          className="mt-6 inline-flex items-center gap-2 font-body text-xs font-semibold uppercase tracking-widest text-brown transition-colors group-hover:text-navy"
        >
